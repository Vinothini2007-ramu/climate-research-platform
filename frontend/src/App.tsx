    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="nav">
          ...
          >
            <FolderKanban /> Research Projects
          </button>
        </div>
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        <p className="muted">
          {register
            ? "Join the climate research platform"
    ...
    <section>
      <div className="hero">
        <div>
          ...
          <h2>
            One place for climate data, analysis and collaboration.
          </h2>

          <p>
            Upload datasets, discover research projects and turn
            environmental data into insights.
          </p>
        </div>
      ...
      <div className="panel">
        ...
        <div className="capabilities">
          <div>
            <Database />
            <b>Centralized storage</b>
            <span>
              Manage climate datasets in one secure platform.
            </span>
          </div>

          <div>
            <BarChart3 />
            <b>Interactive analytics</b>
            <span>
              Explore temperature, rainfall and environmental trends.
            </span>
          </div>
          ...
          <div>
            ...
            <span>
              Create projects and share work with research members.
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <section>
      ...
      <div className="panel">
        ...
        {datasets.length === 0 ? (
          <div className="empty">
            No datasets found. Upload your first climate dataset.
          </div>
        ) : (
...
function UploadForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!file) {
      setError("Select a CSV, JSON or TXT file.");
      return;
    }
...
function DatasetAnalytics({
  ...
  useEffect(() => {
    api
      .get(`/datasets/${dataset.id}/analytics`)
  ...
  return (
    <div className="modal-bg">
      <div className="modal large-modal">
        ...
        <p>
          {data?.rowCount ?? "Loading"} rows ·{" "}
          {data?.headers?.length ?? 0} columns
        </p>

        {first && chartData.length > 1 && (
          <div className="chart">
            <h3>{first} trend preview</h3>
    ...
    <section>
      <div className="toolbar">
        <div>
          ...
          <p className="muted">
            Organize datasets, members and research outputs.
          </p>
        </div>
...
function ProjectCard({
  ...
  const [showOutputForm, setShowOutputForm] =
    useState(false);

  const loadOutputs = () => {
    api
      .get(`/projects/${project.id}/outputs`)
      .then((r) => setOutputs(r.data.outputs))
      .catch(() => setOutputs([]));
  };
